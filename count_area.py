...
def three_points_solar(Lng: float, Lat: float) -> list:
...
def three_points_weather(Lng: float, Lat: float) -> list:
    weather = pd.read_csv("./data/weather_station.csv")[
        ["StationName", "StationLongitude", "StationLatitude"]
    ]  # 氣象站
    ...
    for i in range(len(weather)):
        dis = cal_dis(
            Lng, Lat, weather["StationLongitude"][i], weather["StationLatitude"][i]
        )
        dis = float(dis)
        if (dis < temp[0][1]) | (dis < temp[1][1]) | (dis < temp[2][1]):
            index = find_max(temp[0][1], temp[1][1], temp[2][1])
...
def three_points_uva(Lng: float, Lat: float) -> list:

    uva = pd.read_csv("./data/uva_station.csv")[
