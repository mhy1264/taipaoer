...
def get_obv_station(gen_station: str):
    ...
    df = obv_station[
        obv_station["gen_station"].astype(str).str.contains("{}".format(gen_station))
    ]
    # print(df)
    try:
...
def get_data_from_cwb(
    station: str, start: datetime.date, finish: datetime.date
) -> pd.DataFrame:
    ...
    data = {
        "date": "{}-{}-{}T00:00:00".format(
            start.year,
            start.month if start.month >= 10 else "0{}".format(start.month),
            start.day if start.day > 0 else "0{}".format(start.day),
        ),
        "type": "report_month",
        "stn_ID": str(station_info[0]),
        "stn_type": station_info[1],
        "start": "{}-{}-{}T00:00:00".format(
            start.year,
            start.month if start.month >= 10 else "0{}".format(start.month),
            start.day if start.day > 0 else "0{}".format(start.day),
        ),
        "end": "{}-{}-{}T00:00:00".format(
            finish.year,
            finish.month if finish.month >= 10 else "0{}".format(finish.month),
            finish.day if finish.day > 0 else "0{}".format(finish.day),
        ),
    }

    # print(data)
    res = requests.post("https://codis.cwa.gov.tw/api/station?", data=data)
    ...
    df = pd.DataFrame(
        columns=["date", "Temp", "UV", "SunShineHour", "GlobalRad", "notNull"]
    )
    ...
    if ndata != 0:
        ...
        for i in daily_data:
            ...
            temp[2] = (
                i["UVIndex"]["Maximum"]
                if ("UVIndex" in i and "Maximum" in i["UVIndex"])
                else -1
            )
            temp[3] = (
                i["SunshineDuration"]["Total"]
                if ("SunshineDuration" in i and "Total" in i["SunshineDuration"])
                else -1
            )
            temp[4] = (
                i["GlobalSolarRadiation"]["Accumulation"]
                if (
                    "GlobalSolarRadiation" in i
                    and "Accumulation" in i["GlobalSolarRadiation"]
                )
                else -1
            )
            temp[5] = True
...
def get_data(station: str) -> pd.DataFrame:
    ...
    for year in range(2017, 2023):
        ...
        for month in range(1, 13):
            df = get_data_from_cwb(
                station,
                datetime.date(year, month, 1),
                datetime.date(year, month, ds[month]),
            )
    ...
    for month in range(1, 7):
        df = get_data_from_cwb(
            ...
            datetime.date(2023, month, ds[month]),
        )
        temp = pd.concat([temp, df])
    return temp
    ...
    avedf = pd.DataFrame(
        {
            ...
            "SunShineHour": pd.DataFrame(df["SunShineHour"] / nVaild["vaild"])
            .dropna()[0]
            .to_list(),
            "GlobalRad": pd.DataFrame(df["GlobalRad"] / nVaild["vaild"])
            .dropna()[0]
            .to_list(),
...
#     combined = combine(w1, w2, w3)
#     return (w1, w2, w3, combined)
...
def get_history_data(station_name: list, offset: int):
    ...
    d0 = get_data_from_cwb(station_name[0], start, finish)
    d1 = get_data_from_cwb(station_name[1], start, finish)
    d2 = get_data_from_cwb(station_name[2], start, finish)
...
if __name__ == "__main__":
    ...
    sta1, sta2, sta3, comb = get_history_data(["臺北", "臺中", "高雄"], 30)
    print(sta1.head())
    print(sta2.head())
    print(sta3.head())
    print(comb.head())
